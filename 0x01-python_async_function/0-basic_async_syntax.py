#!/usr/bin/env python3

"""
This module contains an asynchronous coroutine that waits for a random delay
between 0 and max_delay (included and float value) seconds and then returns it.
"""

import asyncio
import random


async def wait_random(max_delay: int = 10) -> float:
    """
    Wait for a random delay between
    0 and max_delay (included and float value) seconds

    Args:
        max_delay (int): The maximum number of seconds
        to wait for (default 10).

    Returns:
        float: A float representing the amount of time waited.
    """
    delay = random.uniform(0, max_delay)
    await asyncio.sleep(delay)
    return delay
